...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const db = getDb()
  const note = db.prepare('SELECT seller_id FROM notes WHERE id = ?').get(params.id) as any
  if (!note) return NextResponse.json({ error: 'Not bulunamadı' }, { status: 404 })
  ...
  db.prepare('UPDATE notes SET title = ?, description = ?, course = ?, instructor = ? WHERE id = ?')
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const db = getDb()
  const note = db.prepare('SELECT seller_id, file_path FROM notes WHERE id = ?').get(params.id) as any
  if (!note) return NextResponse.json({ error: 'Not bulunamadı' }, { status: 404 })
  if (note.seller_id !== session.id) return NextResponse.json({ error: 'Bu notu silme yetkin yok' }, { status: 403 })

  db.prepare('DELETE FROM reviews WHERE note_id = ?').run(params.id)
  db.prepare('DELETE FROM favorites WHERE note_id = ?').run(params.id)
  db.prepare('DELETE FROM ai_summaries WHERE note_id = ?').run(params.id)
