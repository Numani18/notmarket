...
export async function PATCH(req: NextRequest) {
  ...
  const db = await getDb()
  const note = await db.prepare('SELECT id, file_path FROM notes WHERE id = ?').get(noteId) as any
  ...
  if (action === 'hide') {
    ...
    await db.prepare("UPDATE notes SET status = 'active', report_count = 0 WHERE id = ?").run(noteId)
    await db.prepare("UPDATE reports SET status = 'resolved' WHERE note_id = ?").run(noteId)
  } else if (action === 'delete') {
    await db.prepare('DELETE FROM reports WHERE note_id = ?').run(noteId)
    await db.prepare('DELETE FROM favorites WHERE note_id = ?').run(noteId)
    await db.prepare('DELETE FROM ai_summaries WHERE note_id = ?').run(noteId)
